...
export default function GeneratePage() {
  ...
  const norm = (n) => {
    const params = getValue(n, ['params', 'Params', 'arguments', 'args', 'parameters'], []);
    return {
      name: getValue(n, ['name', 'NativeName', 'hashName', 'nativeName', 'fnName'], ''),
      hash: getValue(n, ['hash', 'Hash', 'native', 'Native', 'hashValue'], ''),
      comment: getValue(n, ['comment', 'desc', 'description', 'summary'], ''),
      params: Array.isArray(params) ? params.map((p, i) => ({
        type: getValue(p, ['type', 'Type', 'paramType', 'dataType'], 'Any'),
        name: getValue(p, ['name', 'Name', 'paramName', 'varName'], `p${i}`)
      })) : [],
      returns: getValue(n, ['returns', 'return', 'return_type', 'returnType', 'Returns', 'returnTypeName', 'results'], 'Any')
    };
  };
  ...
  const generateHpp = () => {
    ...
    try {
      ...
      selectedNamespaces.forEach(ns => {
        const nativeEntries = Array.isArray(nativesMap[ns])
          ? nativesMap[ns].map((n, index) => [String(index), n])
          : Object.entries(nativesMap[ns] || {});

        hppContent += `namespace ${ns} {\n`;
        nativeEntries.forEach(([hashKey, n]) => {
          const nativeObj = n && typeof n === 'object' ? n : { name: n };
          const nativeName = getValue(nativeObj, ['name', 'NativeName', 'hashName', 'fnName'], 'native');
          const nativeHash = getValue(nativeObj, ['hash', 'Hash', 'native', 'Native', 'hashValue'], hashKey || '0x0');
          const nativeReturnType = getValue(nativeObj, ['returns', 'return', 'return_type', 'returnType', 'results'], 'Any');

          let processedParams = Array.isArray(nativeObj.params)
            ? nativeObj.params.map((p, i) => ({
                type: getValue(p, ['type', 'Type', 'paramType', 'dataType'], 'Any'),
                name: getValue(p, ['name', 'Name', 'paramName', 'varName'], `p${i}`)
              }))
            : [];
          ...
          const returnType = String(nativeReturnType || 'Any');
          const funcName = formatName(nativeName || 'native');
          const resolvedHash = String(nativeHash || '0x0');

          hppContent += `    // ${nativeName} | ${resolvedHash}\n`;
          hppContent += `    static ${returnType} ${funcName}(${params}) {\n`;
          if (returnType.toLowerCase() === 'void') {
            hppContent += `        ${customInvoke}<${resolvedHash}>(${paramNames});\n`;
          } else {
            hppContent += `        return ${customInvoke}<${resolvedHash}, ${returnType}>(${paramNames});\n`;
          }
          hppContent += `    }\n\n`;
        });
        hppContent += `}\n\n`;
      ...
      alert('Failed to generate header. Please check the selected namespaces and data format.');
    } finally {
      setGenerating(false);
    }
  };
